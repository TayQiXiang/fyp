...
class ImageDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.data = []
        self.class_names = []  # To store class names
        self.image_path=[]

        # Load data from JSON file
        json_file = open(os.path.join(root_dir, "metadata.json"), 'r')
        ...
        # get the component_info list
        components = parsed_json["component_info"]

        for item in components:
            try:
                image_name = item["metadata"]["image"].get("Pad")
                class_name = item["metadata"]["subtype"]
                if image_name and class_name not in self.class_names:
        ...
        self.class_names.sort()  # Sort class names for consistency
        self.class_to_idx = {cls_name: idx for idx, cls_name in enumerate(self.class_names)}  # Map class name to index
    ...
    def __getitem__(self, index):
        ...
        # Convert class name to one-hot vector
        class_index = self.class_to_idx[class_name]
        one_hot = torch.zeros(len(self.class_names))
        one_hot[class_index] = 1

        return image, one_hot, index
...
class Phase1Dataset(Dataset):
    def __init__(self,root, transform=None, filename='train',seperate_multiclass=False,ratio=1):
...
def nepcon(**kwargs):
    ...
    d = ImageDataset('data/nepcon', transform=transform)
    return d
